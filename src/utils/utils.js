...
function axiosWrap(config) {
  ...
  function createAxios(config) {
    var CANCEL_TOKEN = axios.CancelToken
    var url = config.url
      ...
      axiosFn
        .then(function (res) {
          ...
          if (res.status == 200) {
            ...
            if (!config.code || (config.code && config.code.includes(data.code))) {
              if (resolve && Type(resolve) == 'function') {
                return resolve(data)
              } else {
                config.success && config.success(data)
...
function Insert(obj, obj1, obj2) {
  //父元素，要插入的元素，插入元素的后一个兄弟元素
...
var idCardNo = {
  ...
  getIdCardNoCheckInfo: function (idCardNo) {
    ...
    var idCardNo = this.normalIdCardNo(idCardNo)
    var checkResult = [regTestResult, this.checkAddressCode(idCardNo), this.checkDobCode(idCardNo), this.checkLastCode(idCardNo)]
